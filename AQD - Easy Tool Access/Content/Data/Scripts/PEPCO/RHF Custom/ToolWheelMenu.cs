...
namespace PEPCO
{
    ...
    public class ToolWheelMenu : HudElementBase
    {
        ...
        protected override void HandleInput(Vector2 cursorPos)
        {
            ...
            bool shiftPressed = MyAPIGateway.Input.IsKeyPress(MyKeys.LeftShift) || MyAPIGateway.Input.IsKeyPress(MyKeys.RightShift);
            ...
            if (sel >= 0)
            {
                ...
                if (scroll != 0)
                {
                    if (SelectionMode == UserConfigSettings.WheelSelectionMode.MouseWheelScroll)
                        {
                            // Mouse Wheel Scroll mode: scroll rotates wheel selection OR cycles variants if Shift is pressed
                            if (shiftPressed && hoveredEntry != null && hoveredEntry.Available && hoveredEntry.Variants != null && hoveredEntry.Variants.Count > 1)
                            {
                                // Shift+Scroll: cycle variants on current selection
                                if (scroll > 0)
                                {
                                    hoveredEntry.CycleVariant(1);   // Scroll up = next variant
                                    RefreshHighlight();
                                }
                                else if (scroll < 0)
                                {
                                    hoveredEntry.CycleVariant(-1);  // Scroll down = previous variant
                                    RefreshHighlight();
                                }
                            }
                            else
                            {
                                // No Shift or no variants: rotate wheel selection
                                int currentIndex = sel;
                                int nextIndex = currentIndex;

                                if (scroll > 0)
                                {
                                    // Scroll up = clockwise = next slot
                                    nextIndex = (currentIndex + 1) % _wheel.EntryList.Count;
                                }
                                else if (scroll < 0)
                                {
                                    // Scroll down = counter-clockwise = previous slot
                                    nextIndex = currentIndex - 1;
                                    if (nextIndex < 0)
                                        nextIndex = _wheel.EntryList.Count - 1;
                                }

                                // Skip dummy/disabled entries when rotating
                                int attempts = 0;
                                while (attempts < _wheel.EntryList.Count)
                                {
                                    var nextEntry = _wheel.EntryList[nextIndex] as ToolEntry;
                                    if (nextEntry != null && nextEntry.Enabled && !nextEntry.IsDummy)
                                        break;

                                    if (scroll > 0)
                                    {
                                        nextIndex = (nextIndex + 1) % _wheel.EntryList.Count;
                                    }
                                    else
                                    {
                                        nextIndex--;
                                ...
                                if (nextIndex != currentIndex)
                                    _wheel.SetSelectionAt(nextIndex);
                            }
                        }
                    else
                    {
                        // DirectionalFlick and CursorTracking modes: scroll only cycles variants
        ...
        private void RefreshHighlight()
        {
            ...
            if (SelectionMode == UserConfigSettings.WheelSelectionMode.MouseWheelScroll)
            {
                variantHint = hasMultipleVariants ? "\nScroll+Shift to cycle variants" : "\nScroll to rotate wheel";
            }
            else if (SelectionMode == UserConfigSettings.WheelSelectionMode.CursorTracking)
            {
                variantHint = hasMultipleVariants ? "\nScroll to cycle variants" : "\nMove cursor to select";
            }
            else
            {
                // DirectionalFlick mode
                variantHint = hasMultipleVariants ? "\nScroll to cycle variants" : "";
            }
